...
\begin{itemize}
    ...
    \item \textbf{C3 (Selection Policy)}: A multi-objective question selection policy conditioned on knowledge gaps, resume priors, coverage state, information value, and evaluator next-direction hints, with optional IRT-inspired and UCB-based enhancements (Section~\ref{sec:selection}).
    ...
    \item \textbf{C5 (Tool Architecture)}: An extensible tool-augmented architecture where the interview agent invokes specialized tools---resume parser, follow-up generator, speech/expression analyzers, report synthesizer---each with graceful degradation (Section~\ref{sec:tools}).
\end{itemize}
...
The current implementation approximates this objective through a three-tier priority system that operates on the state $s_t = (d_t, C_t, \mathcal{M}_t, R, E_t, B_t)$:
...
Report synthesizer & LLM-enhanced personalized assessment report & $\rightarrow$ Rule-based report \\
\bottomrule
\end{tabularx}
\end{table}

\noindent \textbf{Resume parser.} Extracts structured information from PDF and DOCX documents using hierarchical parsing \cite{li2022resume}. Education, experience, projects, and skills are structured into JSON with confidence scores. Fuzzy matching (70\% threshold) enables robust skill-to-chapter alignment for the selection policy.
...
\noindent \textbf{Report synthesizer.} Produces comprehensive assessment reports with optional LLM enhancement. Rule-based baseline aggregates scores and identifies strengths/weaknesses. LLM enhancement (when available) adds: (1)~personalized overall summary, (2)~context-aware strengths/weaknesses, (3)~actionable learning plan, (4)~speech improvement recommendations, and (5)~question rationales. Each LLM call validates output and falls back to rule-based generation on failure.
...
\begin{itemize}
    ...
    \item \textbf{State-conditioned question selection} (Section~\ref{sec:selection}): the multi-priority policy uses gap state, resume prior, coverage state, and evaluator hints from memory.
    ...
    \item \textbf{Tool-augmented architecture} (Section~\ref{sec:tools}): specialized tools (resume parser, follow-up generator, speech/expression analyzers, report synthesizer) each with defined interfaces and fallback behavior.
\end{itemize}
