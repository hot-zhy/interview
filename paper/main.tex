...
\begin{itemize}
    ...
    \item \textbf{C4 (Learner-centered evaluation)}: An evaluation axis foregrounding candidate/student perceived outcomes and experience, alongside technical metrics for the hybrid judge (routing, validation, fallback) treated as an \emph{online evaluation module} supporting the loop rather than a standalone headline contribution (Sections~\ref{sec:experiments}--\ref{sec:results}).
\end{itemize}
...
\noindent The experimental evaluation is organized around four evaluation axes aligned with RQ1--RQ3 and technical validation of the online judge:

\begin{enumerate}
    \item \textbf{Joint adaptive policy (RQ1):} Do coupled difficulty/termination control and state-conditioned selection (including $\eta_t$ and LLM topic planning when enabled) calibrate within a limited budget and balance gap targeting, coverage, and personalization relative to baselines?
    \item \textbf{Session-level report pipeline (RQ2):} Does the multi-step decomposition-and-analysis pipeline (Table~\ref{tab:report_steps}) produce coherent narrative, dimensional, diagnostic, and transparency outputs under LLM availability constraints, with graceful rule fallbacks?
    \item \textbf{Learner-centered outcomes (RQ3):} What are participants' perceived usefulness, readiness/confidence, fairness, and intent to reuse? We position findings relative to prior AI mock-interview HCI studies \cite{conversate2024,virtual_interviewers2025}; metrics remain illustrative until final field data replace synthetic tables.
    \item \textbf{Hybrid judging (technical validation):} Does routing + validation + fallback improve robustness and agreement with human evaluators vs.\ rule-only scoring? Does multi-judge aggregation reduce variance when enabled?
\end{enumerate}
...
\noindent \textbf{Summary (RQ1):} Jointly, the control and selection halves of the policy operate on shared interview memory: the controller achieves rapid difficulty convergence within 4--5 questions (sliding window, trend-aware updates), while the state-conditioned selection policy balances gap targeting, personalization, and coverage through the priority cascade and evaluator hint augmentation ($\eta_t$). Control effectiveness is robust to heuristic vs.\ target-score strategies; selection balance emerges from memory-driven priority logic rather than fixed weights.

\subsection{Session-Level Report Pipeline (RQ2)}
\label{sec:results_report}

\noindent Qualitative inspection of generated reports (with LLM API access enabled) shows that the five-step pipeline (Table~\ref{tab:report_steps}) surfaces distinct analytic roles: Step~1 anchors narrative in per-question scores; Step~2 yields structured per-dimension rationales; Steps~3--4 translate accumulated missing concepts into diagnostic and learning guidance; Step~5 narrates difficulty/chapter trajectories for transparency. Step-wise fallbacks populate markdown with rule-based strengths/weaknesses when LLM calls fail, preserving a usable artifact. \textbf{Automated metrics} for report quality (e.g., rubric-based human rating of faithfulness and actionability) are left for future work pending annotated gold reports.

\subsection{Learner-Centered Outcomes (RQ3)}
\label{sec:results_user}

\noindent Table~\ref{tab:participants} lists illustrative demographics. \textbf{Target instruments} include post-session usefulness and ease-of-use (SUS-style items), self-reported readiness/confidence for technical interviews, perceived fairness, and behavioral intention to reuse---aligned with themes in prior AI mock-interview HCI work \cite{conversate2024,virtual_interviewers2025}. Numeric outcome tables are placeholders until full recruitment and IRB-approved deployment replace illustrative values; learner-centered claims should be read as \emph{design-level commitments} alongside the technical metrics above.
...
\noindent \textbf{Summary (hybrid judging):} The reliability-constrained hybrid judging framework achieves significantly higher agreement with human evaluators than rule-only evaluation, with the routing and fallback mechanism ensuring continuous operation. The largest improvements occur in dimensions requiring nuanced understanding, confirming the value of validated LLM augmentation.

\subsection{Memory-Aware Integration}

\noindent We analyze the integrated system to assess whether the memory-aware orchestration---where joint adaptive control/selection and reliability-constrained judging operate on shared interview memory---produces benefits beyond the sum of individual components.
...
\noindent \textbf{Summary (Integration):} The integrated results demonstrate that joint adaptive control/selection, session-level reporting, and hybrid judging operate coherently through shared interview memory. Current results primarily demonstrate component effectiveness; full end-to-end superiority over baselines requires the planned ablation experiments.
...
\begin{itemize}
    ...
    \item \textbf{Joint adaptive policy} (Sections~\ref{sec:adaptive_control}--\ref{sec:selection}): difficulty update and termination use sliding-window performance aggregation from memory; question selection uses the same memory for multi-objective priorities, optional LLM-as-topic-planner validation, and $\eta_t$-augmented gap targeting.
    ...
    \item \textbf{Session-level multi-step report generation} (Section~\ref{sec:report_pipeline}): hierarchical decomposition of the full session (Table~\ref{tab:report_decomposition}) followed by five sequential analysis steps (Table~\ref{tab:report_steps}) with independent fallbacks.
...
\noindent \textbf{RQ1 (Joint adaptive policy):} Sections~\ref{sec:adaptive_control}--\ref{sec:selection} specify a coupled policy on shared memory. Empirically, memory-conditioned sliding-window calibration converges within 4--5 questions without IRT calibration, and selection exhibits balanced priority application (Section~\ref{sec:results_rq1}, \ref{sec:results_selection}). Strategy choice (heuristic vs.\ target-score) chiefly affects control dynamics; the evaluator-to-selection loop ($\eta_t$ augmenting $\mathcal{M}_t$) ties judging signals to the next topic.

\noindent \textbf{RQ2 (Session-level report analysis):} The report pipeline hierarchically decomposes the session and runs five analysis passes over a shared compressed context (Tables~\ref{tab:report_decomposition}--\ref{tab:report_steps}), with step-wise fallbacks (Section~\ref{sec:results_report}). Faithfulness metrics await annotated evaluation.

\noindent \textbf{RQ3 (Learner-centered outcomes):} Intended measures (usefulness, readiness/confidence, fairness, reuse) follow HCI practice \cite{conversate2024,virtual_interviewers2025}; Section~\ref{sec:results_user} holds placeholders until field data are available.

\noindent \textbf{Hybrid judging (technical validation):} Routing + validation + fallback improves agreement with human evaluators over rule-only scoring (Section~\ref{sec:results_hybrid}), with automatic fallback preserving uptime.
...
Our design deliberately constrains LLM usage to evaluation, follow-up generation, optional topic planning, and session-level report steps (where nuance adds value) while keeping core control logic (difficulty update, termination, selection priority cascade) deterministic. This hybrid approach preserves the benefits of LLM sophistication for subjective tasks while maintaining the reproducibility required for high-stakes assessment.
...
\begin{itemize}
    ...
    \item \textbf{Illustrative results}: numeric values are computed from synthetic data and are illustrative until recomputed from real participant data (see \texttt{reproduce\_results/}); learner-centered outcome tables are explicitly pending (Section~\ref{sec:results_user}).
...
\noindent Technical interviews are a high-stakes gatekeeper in software hiring, yet most AI-powered systems replicate the same failures at scale: one-size-fits-all difficulty, context-blind question sequences, and evaluation that is either shallow or unreliable. We argued that adaptive interviewing is fundamentally a sequential decision problem and presented a memory-aware, tool-augmented agentic interview framework that addresses core design gaps through a closed-loop observation--memory--policy--action process.

\noindent \textbf{Summary of contributions.} \textbf{C1:} We specified the agentic loop with persistent memory and tool routing. \textbf{C2:} We formalized a joint adaptive policy coupling difficulty/termination with state-conditioned selection (including $\eta_t$ and LLM-as-topic-planner fallbacks) on shared memory. \textbf{C3:} We described session-level report analysis via trace decomposition and five sequential LLM steps with independent fallbacks. \textbf{C4:} We foreground learner-centered evaluation (Section~\ref{sec:results_user}) alongside technical validation of hybrid judging (Section~\ref{sec:results_hybrid}). The online judge is a supporting module, not a standalone headline contribution.

\noindent \textbf{Claim boundary.} We describe the system as a domain-specific, memory-aware, tool-augmented adaptive interviewer with structured control logic and constrained LLM usage. It is not a fully autonomous general-purpose LLM agent. The action space is fixed, core control and priority logic are deterministic, and LLM components operate under validation and fallback constraints. Many numeric results are illustrative (synthetic reproduction); learner-outcome claims await field data.
