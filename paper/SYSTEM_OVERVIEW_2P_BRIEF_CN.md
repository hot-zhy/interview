...
### 4.2.1 语音、视频与表情分析（辅助信号）
- **输入**
  - 语音：音频时长、停顿、转写文本、识别置信度
  - 视频/表情：实时视频分析结果（`analyses` 列表）或单帧图片
- **怎么做**
  - 语音模块把音频转写后计算：语速、停顿频率、平均停顿时长、流畅度、紧张度
  - 表情模块用 DeepFace 输出情绪分布，再映射成面试相关指标：紧张度、自信度、投入度
  - 实时视频模式会把多帧分析写入会话历史，评估时取最近帧，报告阶段再做全局聚合趋势
- **输出到哪里**
  - 每题评估记录：`speech_analysis_json`、`expression_analysis_json`
  - 会话级历史：`expression_history_json`（用于报告趋势）
- **失败时怎么回退**
  - 语音服务不可用：保留文本主流程
  - 表情/视频不可用：跳过该信号，不影响评估、选题和终止主链路
- **怎么评估**
  - 语音：紧张度与人工标注相关性、流畅度相关性
  - 表情：情绪识别准确率（如 FER 数据集）+ 面试场景解释一致性
  - 工程：多模态关闭时端到端流程成功率（降级可用性）
...
- 多模态辅助：看语音/表情指标与人工标注相关性，以及关闭多模态后的主流程可用性
