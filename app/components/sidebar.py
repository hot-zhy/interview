...
def render_sidebar():
    ...
    with st.sidebar:
        ...
        if st.session_state.get("authenticated"):
            ...
            st.page_link("pages/2_Resume.py", label=f"📄 {t('app.resume')}")
            st.page_link("pages/3_QuestionBank.py", label=f"📚 {t('app.question_bank')}")
            st.page_link("pages/4_Interview.py", label=f"💼 {t('app.interview')}")
            st.page_link("pages/5_Report.py", label=f"📊 {t('app.report')}")
            st.page_link("pages/6_Admin.py", label=f"⚙️ {t('app.admin')}")
        else:
            st.page_link("pages/1_Auth.py", label=f"🔐 {t('app.login_register')}")
        st.markdown("---")
